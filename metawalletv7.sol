...
contract MetaWalletV7 {
    ...
    // EIP-712 constants
    bytes32 public constant META_TX_TYPEHASH = keccak256("MetaTransaction(address user,address target,bytes data,uint256 value,uint256 nonce)");
    ...
    event MetaTransactionExecuted(address indexed user, address target, uint256 value, bytes data);
    ...
    function executeMetaTransaction(
        ...
        emit MetaTransactionExecuted(user, target, value, data);
    }
...
contract UserWallet {
    ...
    function execute(address target, bytes memory data, uint256 value) external onlyOwner returns (bool) {
